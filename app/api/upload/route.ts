...
import { FILE_TYPES, uploadType, type FileEntry } from '@/lib/types';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!type) {
            return NextResponse.json({ error: 'Only JPG, PDF, and Word (.doc or .docx) files are accepted.' }, { status: 400 });
        }
        ...
        const ext = FILE_TYPES[type].extension;
